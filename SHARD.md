...
## Часть 19: H5 — bypass-fsync на реальном сервере

**Дата:** 2026-05-25  
**Сервер:** lotos-archive-01, 48 ядер, 256 GB RAM  
**Scylla:** Docker 6.2, smp=32, memory=128G, `developer-mode=1`, **реальный диск**, `--unsafe-bypass-fsync 1`  
**Парсер:** zigparser2_p48, pool=48, split=1,4,8,30,3,2  
**Чанкование:** chunk = block_number / 1000

### Результаты

| Конфиг | save avg | save min | save max | total avg |
|--------|---------|---------|---------|---------|
| pool=48, fsync (Часть 18) | 17.0 ms | 10.5 ms | 35.1 ms | 25.3 ms |
| **pool=48, bypass-fsync** | **18.3 ms** | **9.3 ms** | **35.9 ms** | **26.9 ms** |

### Вывод: bypass-fsync не помог

Результат практически идентичен — в пределах погрешности. Гипотеза о fsync как bottleneck **не подтвердилась**.

**Объяснение пути записи Scylla:**
```
Client → [1. Commitlog append] → [2. Memtable write] → ACK клиенту
                                        ↓ (фоновый flush)
                                     Disk (fsync опционально)
```
ACK отправляется после шага 2 (memtable), **до fsync**. bypass-fsync убирает лишь фоновый сброс на диск — он не влияет на latency write в критическом пути.

**Реальный bottleneck:** memtable-операции на одном шарде Scylla. Все 100 блоков → chunk 25079 → **один шард** обрабатывает все ~30 батчей блока последовательно (reactor model). Это CPU-bound операция: сортировка строк, обновление Bloom filter, partition index.

**Почему WSL tmpfs было быстрее (8.5ms):** не из-за bypass-fsync, а потому что tmpfs = RAM → memtable flush мгновенный, нет конкуренции за I/O scheduler.

### Риски bypass-fsync в production

| Аспект | Оценка |
|--------|--------|
| Потеря данных при crash | До последнего фонового fsync (секунды–минуты) |
| Для blockchain-парсера | Приемлемо (переиндексация с checkpoint) |
| Для финансовых/критичных данных | Неприемлемо |
| Выигрыш latency | **0 ms** (не подтвердился) |
