...
блока по формуле **"lane + era"** (v3-схема, текущая):
...
Все таблицы партиционированы по `chunk` (кроме `contracts_by_addresses`, см. ниже). Суффиксы `_v2`/
`_v3` — таблицы от прошлых, незавершённых попыток миграции схемы, **не используются** активным
индексером сейчас (можно игнорировать при чтении, актуальные данные — в таблицах без суффикса).
...
| `*_v2`, `*_v3` | (та же структура) | Устаревшие, от прошлых попыток миграции на новую chunk-схему, не используются — можно дропнуть |
