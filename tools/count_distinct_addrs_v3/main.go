// count_distinct_addrs_v3: counts unique addresses in contracts_by_address_v2.
// Uses SELECT address GROUP BY address per segment (one row per partition key)
// with a bounded worker pool of 16 workers. Avoids the gocql paging hang that
// occurs when all 256 goroutines scan simultaneously.
package main
...
func main() {
	...
	for i := 0; i < *workers; i++ {
		...
		go func() {
			defer wg.Done()
			for s := range segCh {
				count, err := countSegment(session, s.lo, s.hi, *pageSize, *retries)
				if err != nil {
...
func countSegment(session *gocql.Session, lo, hi int64, pageSize, maxRetries int) (int64, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		count, err := tryCount(session, lo, hi, pageSize)
		if err == nil {
			return count, nil
		}
...
func tryCount(session *gocql.Session, lo, hi int64, pageSize int) (int64, error) {
	// Scan ALL rows (same sequential pattern as COUNT(*) — fast with 104 SSTables).
	// Rows come back in token order (partition order), so unique-address count is
	// just the number of address transitions. O(1) extra memory.
	iter := session.Query(
