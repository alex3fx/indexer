#!/usr/bin/env bash
# Bytecode-store backfill: node .07, blocks 0 → 12 700 000.
# Uses Redis DB=3 (dedicated cursor, doesn't touch DB=2 used by realtime).
# Auto-restarts on crash, resumes from log watermark.
set -uo pipefail

BIN="$HOME/raw_erc20_v12"
LOG="$HOME/eth_backfill_07.log"
TO=12700000
REDIS_DB=3
...
while true; do
    ...
    if [[ -n "$LAST" && "$LAST" -ge "$TO" ]]; then
        echo "$(date -u +%FT%TZ)  Backfill 07 complete at block $LAST" | tee -a "$LOG"
        exit 0
    fi
    FROM=${LAST:-0}
    echo "$(date -u +%FT%TZ)  Starting backfill 07: FROM=$FROM TO=$TO" | tee -a "$LOG"
