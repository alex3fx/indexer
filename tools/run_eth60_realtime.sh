...
# Resume from watermark on restart. Auto-restarts on crash (CqlError and similar transient
# failures) with 5-second cooldown.
#
...
#
# Deployed to ~/run_eth60_realtime.sh on 100.64.0.4.
set -uo pipefail
...
while true; do
    ...
    env \
      ...
      "$BIN" --from="$FROM" >> "$LOG" 2>&1 || true

    echo "[realtime] process exited — restarting in 5s" >> "$LOG"
    sleep 5
done
